...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition, self.corner_visited)
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            children = children + [(self.getNextState(state, action), action,
            self.getActionCost(state, action, self.getNextState(state, action)))]
        self._expanded += 1 # DO NOT CHANGE
        return children
    ...
    def getNextState(self, state, action):
        ...
        if (nextx, nexty) in self.corners and (nextx, nexty) not in corner:
            corner = ((nextx, nexty),) + corner
            #print("reached a corner! corners we have so far:", corner)
        return ((nextx, nexty), corner)
...
def cornersHeuristic(state, problem):
    ...
    height = top - 1
    distance = {}
    distance[0] = max(width, height) + min(width, height)*2
    h = []
    corner_list = list(corners)
    for i in visited_corners:
        corner_list.remove(i)
    temp = corner_list.copy()
    for x in corner_list:
        temp.remove(x)
        perm = permutations(temp)
        dis = mazeDistance(x, state[0], problem.getStartingGameState())
        for y in perm:
            length = len(y)
            perms = []
            for i in range(length - 2):
                perms += [manhattanDistance(y[i], y[i + 1])]
        if len(perms) is not 0:
            h += [dis + min(perms)]
        else:
            h += [dis]
    if len(visited_corners) == 4:
        h = [0]
    return max(h) # Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    dis = []
    for f in foodlist:
        dis += [mazeDistance(state[0], f, problem.startingGameState)]
