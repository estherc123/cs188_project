...
def joinFactors(factors):
    ...
    setsOfUnconditioned = set().union(*setsOfUnconditioned)
    setsOfConditioned = []
    newDomain = {}
    for factor in factors:
        conditioned = factor.conditionedVariables()
        for var in conditioned:
            if var not in setsOfUnconditioned and var not in setsOfConditioned:
                setsOfConditioned.append(var)
        newDomain.update(factor.variableDomainsDict())
    setsOfConditioned = set(setsOfConditioned)
    newFactor = Factor(setsOfUnconditioned, setsOfConditioned, newDomain)

    for assignment in newFactor.getAllPossibleAssignmentDicts():
        result = 1
        for factor in factors:
            result *= factor.getProbability(assignment)
        newFactor.setProbability(assignment, result)
    return newFactor
    "*** END YOUR CODE HERE ***"
